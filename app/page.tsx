...
export default function Home() {
  ...
  const nearbyDistricts = serviceAreas.filter((item) => item.type === "ilce");
  return <main className="dispatch-site">
    ...
    <section className="coverage" id="bolgeler"><div className="coverage-title"><p>HİZMET SAHASI / 03</p><h2>Üsküdar’ın<br />her yönüne.</h2><p>Küplüce merkezli; sahil, yokuşlar, ana arterler ve çevre mahalleler için çağrı alıyoruz.</p></div><div className="coverage-map"><div className="map-grid" /><div className="map-pin pin-main"><i />Küplüce</div><div className="map-pin pin-one">Beylerbeyi</div><div className="map-pin pin-two">Altunizade</div><div className="map-pin pin-three">Çengelköy</div><span className="map-caption">ÜSKÜDAR / İSTANBUL</span></div><div className="coverage-links"><p>YAKIN NOKTALAR</p>{neighborhoods.slice(0, 4).map((area) => <Link href={`/bolgeler/${area.slug}`} key={area.slug}>{area.name}<ArrowUpRight /></Link>)}<p className="nearby-label">ÇEVRE İLÇELER</p>{nearbyDistricts.slice(0, 6).map((area) => <Link href={`/bolgeler/${area.slug}`} key={area.slug}>{area.name} <ArrowUpRight /></Link>)}<Link className="all-areas" href="/hizmet-bolgeleri">Tüm mahalle ve ilçeleri göster <ArrowUpRight /></Link></div></section>
